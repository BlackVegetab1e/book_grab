...
public:
    ...
    /// \return                         成功返回：Socket 句柄。失败返回:错误码, define.h查询.
    ///
    ...
    ///
    /// \brief Service_Start_Calibrate   开始标定关节
    /// \param ArmSocket socket句柄
    /// \param block RM_NONBLOCK-非阻塞，发送后立即返回；RM_BLOCK-阻塞，等待控制器返回设置成功指令
    /// \return 0-成功，失败返回:错误码, rm_define.h查询.
    ///
    RM_SERVICESHARED_EXPORT int Service_Start_Calibrate(SOCKHANDLE ArmSocket, bool block);

    ///
    /// \brief Service_Stop_Calibrate   停止标定关节
    /// \param ArmSocket socket句柄
    /// \param block RM_NONBLOCK-非阻塞，发送后立即返回；RM_BLOCK-阻塞，等待控制器返回设置成功指令
    /// \return 0-成功，失败返回:错误码, rm_define.h查询.
    ///
    RM_SERVICESHARED_EXPORT int Service_Stop_Calibrate(SOCKHANDLE ArmSocket, bool block);

    ///
    /// \brief Service_Get_Calibrate_State 获取标定状态
    /// \param ArmSocket socket句柄
    ...
    /// \return 0-成功，失败返回:错误码, rm_define.h查询.
    ///
    RM_SERVICESHARED_EXPORT int Service_Get_Calibrate_State(SOCKHANDLE ArmSocket, float* joint_state,uint16_t *state,int *time);
    ...
    ///
    /// \brief Service_Set_System_State_Servo 该函数用于打开或者关闭系统状态自动回传，自动回传打开后，以50Hz的频率周期回传系统状态。
    /// \param ArmSocket socket句柄
    /// \param cmd true-开启自动回传，false
    /// \param block RM_NONBLOCK-非阻塞，发送后立即返回；RM_BLOCK-阻塞，等待控制器返回设置成功指令
    /// \return 0-成功，失败返回:错误码, rm_define.h查询.
    ///
    RM_SERVICESHARED_EXPORT int Service_Set_System_State_Servo(SOCKHANDLE ArmSocket, bool cmd, bool block);
    ...
    ///
    /// \brief Service_Add_Waypoint 向机械臂添加路径点，机械臂做多可缓存5000个路径点，待收到开始指令后方可运行。
    /// \param ArmSocket socket句柄
    /// \param joint 目标路径点数组，关节1~6的目标角度，单位：度
    /// \param block RM_NONBLOCK-非阻塞，发送后立即返回；RM_BLOCK-阻塞，等待控制器返回设置成功指令
    /// \return 0-成功，失败返回:错误码, rm_define.h查询.
    ///
    RM_SERVICESHARED_EXPORT int Service_Add_Waypoint(SOCKHANDLE ArmSocket, const float *joint, bool block);

    ///
    /// \brief Service_Start_Waypoint 该函数用于开始运行已经下发的路径点，运行频率按照函数中设置。
    /// \param ArmSocket socket句柄
    /// \param frq 路径点运行频率，最高不超过200Hz，最低50Hz，否则会发生关节速度不连续的情况
    /// \param block RM_NONBLOCK-非阻塞，发送后立即返回；RM_BLOCK-阻塞，等待控制器返回设置成功指令
    /// \return 0-成功，失败返回:错误码, rm_define.h查询.
    /// 注意：开始运行缓存内的轨迹点时，确保当前关节角度与第一个路径点的角度差值不超过3°，否则会报错，该项设置为了保证运行时机械臂出现剧烈抖动。
    RM_SERVICESHARED_EXPORT int Service_Start_Waypoint(SOCKHANDLE ArmSocket, int frq, bool block);

    ///
    /// \brief Service_Clear_Waypoint 清空控制器中所有缓存的路径点
    /// \param ArmSocket socket句柄
    /// \param block RM_NONBLOCK-非阻塞，发送后立即返回；RM_BLOCK-阻塞，等待控制器返回设置成功指令
    /// \return 0-成功，失败返回:错误码, rm_define.h查询.
    ///
    RM_SERVICESHARED_EXPORT int Service_Clear_Waypoint(SOCKHANDLE ArmSocket, bool block);

    ///
    /// \brief Service_Stop_Waypoint 停止正在运行的轨迹点
    /// \param ArmSocket socket句柄
    /// \param block RM_NONBLOCK-非阻塞，发送后立即返回；RM_BLOCK-阻塞，等待控制器返回设置成功指令
    /// \return 0-成功，失败返回:错误码, rm_define.h查询.
    ///
    RM_SERVICESHARED_EXPORT int Service_Stop_Waypoint(SOCKHANDLE ArmSocket, bool block);
    ...
    ///
    /// \brief Service_Timer                轨迹等待
    /// \param ArmSocket            socket句柄
    /// \param time                 等待时间，单位：ms
    /// \param block                RM_NONBLOCK-非阻塞，发送后立即返回；RM_BLOCK-阻塞，等待控制器返回设置成功指令
    /// \return                     0-成功，失败返回:错误码, rm_define.h查询.
    ///
    RM_SERVICESHARED_EXPORT int Service_Timer(SOCKHANDLE ArmSocket, int time,bool block);
    ...
    ///
    /// \brief Service_Get_Log_File                 读取SD卡中的Log文件
    /// \param ArmSocket socket句柄
    /// \param block                        RM_NONBLOCK-非阻塞，发送后立即返回；RM_BLOCK-阻塞，等待控制器返回设置成功指令
    /// \return                             0-成功，失败返回:错误码, rm_define.h查询.
    ///
    RM_SERVICESHARED_EXPORT int Service_Get_Log_File(SOCKHANDLE ArmSocket, bool block);
    ...
    ///
    /// \brief Service_Set_Vehicle                  设置移动平台运动
    /// \param ArmSocket                    socket句柄
    ...
    /// \return                             0-成功，失败返回:错误码, rm_define.h查询.
    ///
    RM_SERVICESHARED_EXPORT int Service_Set_Vehicle(SOCKHANDLE ArmSocket, int speed,int angular,bool block);

//    ///
//    /// \brief Service_Set_Lift                     设置升降平台高度
//    /// \param ArmSocket                    socket句柄
...
//    /// \return                             0-成功，失败返回:错误码, rm_define.h查询.
//    ///
//    RM_SERVICESHARED_EXPORT int Service_Set_Lift(SOCKHANDLE ArmSocket, int height,int speed,bool block);
    ...
    ///
    /// \brief Service_Set_Car_Move             速度开环控制
    /// \param ArmSocket                socket句柄
    ...
    /// \return                         0-成功，失败返回:错误码, rm_define.h查询.
    ///
    RM_SERVICESHARED_EXPORT int Service_Set_Car_Move(SOCKHANDLE ArmSocket, int linear,int angular,bool block);

    ///
    /// \brief Service_Set_Car_Station          站点导航控制
    /// \param ArmSocket                socket句柄
    ...
    /// \return                         0-成功，失败返回:错误码, rm_define.h查询.
    ///
    RM_SERVICESHARED_EXPORT int Service_Set_Car_Station(SOCKHANDLE ArmSocket, int station,int speed,bool block);

    ///
    /// \brief Service_Set_Car_Navigation_Speed     速度导航控制
    /// \param ArmSocket socket句柄
    /// \param speed                        导航速度，-100~100，>0前进，<0后退,0-导航停止
    /// \param block                        RM_NONBLOCK-非阻塞，发送后立即返回；RM_BLOCK-阻塞，等待控制器返回设置成功指令
    /// \return                             0-成功，失败返回:错误码, rm_define.h查询.
    ///
    RM_SERVICESHARED_EXPORT int Service_Set_Car_Navigation_Speed(SOCKHANDLE ArmSocket, int speed,bool block);

    ///
    /// \brief Service_Set_Car_Navigation_Mode      设置导航模式
    /// \param ArmSocket                    socket句柄
    /// \param mode                         导航方式，0-磁条导航，1-开环控制模式
    /// \param block                        RM_NONBLOCK-非阻塞，发送后立即返回；RM_BLOCK-阻塞，等待控制器返回设置成功指令
    /// \return                             0-成功，失败返回:错误码, rm_define.h查询.
    ///
    RM_SERVICESHARED_EXPORT int Service_Set_Car_Navigation_Mode(SOCKHANDLE ArmSocket, int mode,bool block);

    ///
    /// \brief Service_Get_Car_State                查询车体状态
    /// \param ArmSocket                    socket句柄
    ...
    /// \return                             0-成功，失败返回:错误码, rm_define.h查询.
    ///
    RM_SERVICESHARED_EXPORT int Service_Get_Car_State(SOCKHANDLE ArmSocket, int* volume,int* voltage,int* current,int* car_err,int* car_state,int* station);

    ///
    /// \brief Service_Set_Car_Charge               控制车体自动充电
    /// \param ArmSocket                    socket句柄
    /// \param charge_state                 0-解决自动充电模式，1-开始自动充电
    /// \param block                        RM_NONBLOCK-非阻塞，发送后立即返回；RM_BLOCK-阻塞，等待控制器返回设置成功指令
    /// \return                             0-成功，失败返回:错误码, rm_define.h查询.
    ///
    RM_SERVICESHARED_EXPORT int Service_Set_Car_Charge(SOCKHANDLE ArmSocket, int charge_state,bool block);

    ///
    /// \brief Service_Set_Car_Station_Num          设置车体当前站点号
    /// \param ArmSocket                    socket句柄
    /// \param charge_state                 站点号
    /// \param block                        RM_NONBLOCK-非阻塞，发送后立即返回；RM_BLOCK-阻塞，等待控制器返回设置成功指令
    /// \return                             0-成功，失败返回:错误码, rm_define.h查询.
    ///
    RM_SERVICESHARED_EXPORT int Service_Set_Car_Station_Num(SOCKHANDLE ArmSocket, int charge_state,bool block);
    ...
    /// \return                  旋转矩阵
    ///
    RM_SERVICESHARED_EXPORT Matrix Service_Euler2matrix(eul state);
    ...
    /// \return                旋转矩阵
    ///
    RM_SERVICESHARED_EXPORT Matrix Service_Pos2matrix(Pose state);
