...
def run(
        weights= pwd+'/hh/best.pt',  # model path or triton URL
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                if lines is None:
                    ...
                    img_source = cv2.imread(pwd+"/temp/tmp.jpg")
                    cv2.circle(img_source, (x,y), 5, (0,0,255),2 )
                    cv2.imshow("hhshow", img_source)
                    cv2.waitKey(1000)
                    cv2.imwrite(pwd+"/temp/hhsb.jpg", img_source)
...
def parse_opt(path):
    ...
    parser.add_argument('--weights', nargs='+', type=str, default= pwd+'/hh/best.pt', help='model path or triton URL')
